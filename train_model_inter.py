...
def worker_inter_coord(dname, file, cut_radius, m_template):
...
def inter_coord(dname):
    ...
    results = [pool.apply_async(worker_inter_coord, args=(dname, file, cut_radius, m_template)) for file in os.listdir(dname)]
    mcoords = [result.get() for result in results]
...
def main():
    ...
    if data_load == "c":
        ...
        data_input, data_output = inter_coord(input_dir), inter_coord(output_dir) # TODO important bug: output 与 input Ce_anchor 不对应
    ...
    data_output[:, 0:3] = np.where((data_output[:, 0:3] - data_input[:, 0:3] > 0.45), data_output[:, 0:3] - 1, data_output[:, 0:3])
    data_output[:, 0:3] = np.where((data_output[:, 0:3] - data_input[:, 0:3] < -0.45), data_output[:, 0:3] + 1, data_output[:, 0:3])
    ...
    #data_input[:, 5] = 0
    #data_output[:, 5] = 0
    #print(np.where(data_output - data_input > 0.4))
    exit()
