...
if __name__ == "__main__":
    ...
    Test_DIR = os.path.join(root_dir, "test_set/guess")
    Path(Test_DIR).mkdir(exist_ok=True)

    test_ori_dir = os.path.join(Test_DIR, "ori")
    Path(test_ori_dir).mkdir(exist_ok=True)

    test_ML_dir = os.path.join(Test_DIR, "ML-xdat-o")
    ...
    model = load_model("results/CeO2_111_CO_xdat_o.h5")

    iter_flag = True
    test_ML_coor = None
    iter_count = 0
    while iter_flag and iter_count < 1:
